...
func (c *connection) write(mt int, payload []byte) error {
	...
	fmt.Printf("\nAddress of variable: %v", &actualVar  ) //0blabla
	fmt.Printf("\nAddress stored in pointer variable: %v", pointerVar ) //0blanla
	
	fmt.Printf("\nValue of Actual Variable: %s",actualVar ) // australia
	fmt.Printf("\nValue of Pointer variable: %s",*pointerVar ) // australia*/
	...
	//fmt.Printf("\nAddress of variable: %v", &nomSalon  ) //0blabla
	//fmt.Printf("\nAddress stored in pointer variable: %v", pointerNomSalon ) //0blanla
	
	//fmt.Printf("\nValue of Actual Variable: %s",nomSalon ) // australia
	//fmt.Printf("\nValue of Pointer variable: %s",*pointerNomSalon ) // australia

	//fmt.Printf("\n avant Pointer nom user: %s",*pointerNomUsers ) // australia  
	//fmt.Printf("\npointeurDonnest: %s",*pointerNomSalon ) 
	//fmt.Printf("valeur variable nom salon %d\n",&nomSalon)
	for _, v := range listAllData {
		...
		if ( strings.Contains(v, "nouveau_joueur") && salonExist==0 ){
			...
			*/
			...
			if (len(AllRoom1.rooms) == 0) {
				...
				notes[NamePlayer] = "{admin: true, drawer: false}"
				notes["chats"] = ""
		...
		if ( salonExist==1  && strings.Contains(v, "chat")){
			...
			//fmt.Println("\n  une accollade de moins",AllRoom1.rooms ) 

			// fmt.Println("\n  test 0 le tableau devient une zone accolade",AllRoom1.rooms[0] ) 
			// fmt.Println("\n  test 1 le tableau devient une zone accolade",AllRoom1.rooms[1] ) 
			...
			//notes["Hatim"]
			// notes2:=InformationRoom{AllRoom1.rooms[0] }
			// notes := map[string]string{"zzzz":"zzz"}
			// fmt.Println(notes)
			// for v, item := range AllRoom1.rooms  {
			// 	fmt.Println("chaque v", v)
			// 	fmt.Println("chaque item", item)
	...
	if ( strings.Contains(string(payload), "points")  ){
